...
if($stmt = $conn->prepare('SELECT id,password FROM students1 WHERE user_name=?')){
    ...
    if($stmt->num_rows>0){
        ...
        if($stmt = $conn->prepare('INSERT INTO students1(user_name,password,email,address,contact,syllabus) VALUES (?,?,?,?,?,?)')){
